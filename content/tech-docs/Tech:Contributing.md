---
title: Tech:Contributing
---

`{{ {{Under construction}} }}`

This is currently a draft, originally from CONTRIBUTING.md in mw-config. It will be expanded to include contributing guidance for other repos (mainly our MediaWiki extensions also), and later linked in each repos README.md and CONTRIBUTING.md (so it shows in the message "Be sure to review the contributing guidelines" when creating pull requests) as well. It will also fully replace the current CONTRIBUTING.md in mw-config itself.
...
* To add extensions to *ManageWikiExtensions.php*:
   * First make sure the extension is added for Miraheze in the [mediawiki-repos.yaml file](https://meta.miraheze.org/wiki/github:miraheze/mediawiki-repos/blob/main/mediawiki-repos.yaml) in the miraheze/mediawiki-repos repository.
...
Please note that wiki identifiers are formed by the name of the wiki followed by the `wiki` string, for instance, `'examplewiki'` is the identifier for `example.miraheze.org`. `examplewikiwiki` would be the identifier for `examplewiki.miraheze.org`.
...
----
**[Go to Source &rarr;](https://meta.miraheze.org/wiki/Tech:Contributing)**