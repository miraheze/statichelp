...
The **Speedscope service** is a node/express service running in a docker container on [speedscope211](https://meta.miraheze.org/wiki/Tech:Speedscope211).
...
Note: The extension is currently only installed on beta, so no production requests are sampled for now.
