...
Miraheze is currently migrating to [Cloudflare](Tech:Cloudflare.md) as a replacement for Varnish. Varnish currently only serves traffic for wikis on *.wikitide.org/*.wikitide.net wikis and for wikis that use Miraheze's DNS nameservers.
...
* Client asks [authoritative DNS](Tech:DNS.md) for the IP of a cache proxy. GeoDNS returns the IP of the nearest cache proxy, and the client will use that cache proxy server;
